...
class ProvisioningParser {
    ...
    func parse(data: Data) -> ProvisioningProfile? {
        // Mobileprovision files are CMS/PKCS#7 signed wrappers around a plist
        // We need to find the XML content between <plist and </plist>
           ...
           scanner.scanUpTo("</plist>", into: &plistString) {
            ...
            if let dict = try? PropertyListSerialization.propertyList(from: plistData, options: [], format: nil) as? [String: Any] {
                return ProvisioningProfile(
                    ...
                    bundleIdentifier: dict["Entitlements"] as? [String: Any]? != nil ? (dict["Entitlements"] as! [String: Any])["application-identifier"] as? String ?? "" : "",
