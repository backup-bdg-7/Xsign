...
class BinaryParser {
    ...
    func getDylibs(at url: URL) -> [String] {
        ...
        let dylibsPtr = c_zsign_get_dylibs(path)
        guard let dylibsPtr = dylibsPtr else { return [] }
    ...
    func getMetadata(appFolder: URL, outputDir: URL, ipaFile: URL? = nil) -> [String: Any]? {
        ...
        let ipaPath = ipaFile?.path ?? ""

        // Call C function
        let metadataPtr = c_zsign_get_metadata(appPath, outputPath, ipaPath)
        guard let metadataPtr = metadataPtr else { return nil }
